...
export function AppHeader() {
  ...
  useEffect(() => {
    const isAdmin = user?.role === 'admin' || (typeof user?.role === 'object' && user.role && (user.role as any).name === 'Admin');
    if (isAdmin) {
      fetchNotifications();
      ...
      socket.on("new_notification", (notification: Notification) => {
        ...
        toast(notification.title, {
          ...
          className: `border-l-4 ${getNotificationColor(notification.type).replace('bg-', 'border-')}`,
        });
      });
    ...
    <header className="sticky top-0 z-50 w-full border-b border-border bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/60">
      <div className="flex h-16 items-center gap-4 px-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {(user?.role === 'admin' || (typeof user?.role === 'object' && user.role && (user.role as any).name === 'Admin')) && (
              ...
              <DropdownMenuLabel>
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {user?.role === 'admin' || (typeof user?.role === 'object' && user.role && (user.role as any).name === 'Admin') ? 'Administrator' : 'Billing User'}
                  </span>
                </div>
              </DropdownMenuLabel>
              <DropdownMenuSeparator />
