...
export default function Settings() {
  ...
  const handleDownloadBackup = async () => {
    try {
      ...
      toast.success("Backup Downloaded", { description: "Database backup saved successfully" });
    } catch (error) {
      toast.error("Error", { description: "Failed to download backup" });
    }
  };
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        toast.success("Restore Successful", { description: "Database restored successfully" });
      } catch (error) {
        toast.error("Error", { description: "Failed to restore backup" });
      }
    };
  ...
  const handleResetDatabase = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/backup/reset`, {
        method: "DELETE",
        credentials: "include",
      });
      if (!response.ok) throw new Error("Reset failed");
      toast.success("Reset Successful", { description: "Database has been reset" });
